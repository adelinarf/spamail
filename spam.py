...
class testData:
	...
	def __init__(self):
		...
		self.train_data = list(map(lambda xs: getRandomSample(xs,0.99),self.test))
...
def naive_bayes(message):
	...
	q=classify(message,words_spam,words_not_spam,alpha,N_spam,N_vocabulary,P_spam,P_not_spam,N_not_spam,message_spam,message_not_spam)
	print (q)
	return (q)
	
       
def classify(message,words_spam,words_not_spam,alpha,N_spam,N_vocabulary,P_spam,P_not_spam,N_not_spam,message_spam,message_not_spam):
	p_spam_message=P_spam
	p_not_spam_message=P_not_spam
	for word in message:
                p_spam_message     *= p_wi_(word,words_spam,message_spam,alpha,N_vocabulary,N_spam)
                p_not_spam_message *= p_wi_(word,words_not_spam,message_not_spam,alpha,N_vocabulary,N_not_spam)
